...
Our primary focus in this analysis centers on evaluating the impact of the STAR project on the academic performance of Grade 1 students, specifically in mathematics. To gauge this, as a start we will rely on the variable "g1tmathss" denoting the Grade 1 SAT scores, "g1classtype" denoting the class type students assigned in, and "g1schid" indicating the school ID. We found that there are 4772 missing values for g1classtype, and 5003 missing values for g1tmathss variable. This prompting a thorough investigation into the underlying causes.

Among the 11,601 observations in the entire data set, only 6,829 students engaged with STAR during Grade 1, while the remaining 4,772 did not partake in the program during Grade 1. Further analysis revealed that 3,537 of these students only participated in the project for 1 year, with varying entry points (1669 participated during kindergarten, 585 participated during the second grade, and 1283 participated during the 3rd grade), while 1,161 were in STAR for 2 years and 74 joined for 3 years, missing only the first grade.

We also observed that 36.4% of the students who joined Star in kindergarten either stopped participating in the program or came back again later for grade 2 or 3. However, a huge percentage, 96%, did not come back to star schools after kindergarten. To explore this further, we compared data between students who continued in Project Star for 2 consecutive years from kindergarten to grade 1 and those who did not. Namely, we inspect students’ data (gender, race, socioeconomic status), school data (urbanicity, class, size), teachers’ data (gender, race, and experience), as well as days school attended. All the numbers and data seemed to be distributed similarly. We observed average class size: no return = 20.69, return = 20.17, and although average days present at school kindergarten for those who return are 159.2 and no return = 149.1, they are still similarly distributed. Therefore, given the absence of any noticeable pattern, the reason students left STAR schools after kindergarten cannot be explained by the aforementioned variables. Hence, we can conclude that the 4772 missing data are due to the fact that the students in the corresponding rows are not in STAR during grade 1.
...
Imputing missing race data using the mode of each class involves leveraging the most frequently occurring race category within each distinct class or educational grouping. This method entails identifying different classes through teacher ID, and calculating the mode of the race category, for each class. Once the modes are determined, missing race values within each class are then replaced with the corresponding mode for that particular class. By employing the mode of each class for imputation, we ensure that missing race values are imputed with the most prevalent racial category observed within specific classroom. This approach acknowledges potential variations in racial composition across different classes or cohorts, thereby capturing the diversity and heterogeneity within the data set.

Justifying the use of mode imputation for race data rests on several key considerations. Firstly, as race is a categorical variable, imputing missing values with the mode aligns with the fundamental principles of categorical data analysis. The mode represents the most frequent category observed in the dataset, providing a logical choice for imputing missing categorical values. Furthermore, using class-specific modes for imputation acknowledges the contextual relevance of race within educational settings. Different classes or cohorts may exhibit distinct racial compositions, and imputing missing race values based on class-specific modes ensures that imputed data closely reflect the prevailing racial demographics within each educational context. This method also simplifies the imputation process by leveraging readily available information within the dataset, making it practical and efficient for handling missing race data.
...
Imputing missing free lunch information using the mode of each school involves utilizing the most frequently occurring free lunch status within each distinct school. This approach is adopted due to the presence of two classrooms with entirely missing free-lunch information, specifically for teacher IDs 20345206 and 22157106, which both represent regular and aide classes with 24 students each situated in a rural location. Since individual classroom-level free lunch data is unavailable for these classrooms, aggregating the free lunch status at the school level provides a feasible alternative. By calculating the mode, or the most common free lunch status, for each school, missing free lunch values within each school can be imputed with the corresponding mode for that particular school.

Justifying the use of school-level mode imputation for free lunch data is warranted based on several considerations. Firstly, given the absence of free lunch information for specific classrooms and the homogeneous characteristics of the classrooms in question (both being regular + aide classes with similar student demographics and situated in rural areas), aggregating data at the school level provides a pragmatic solution. Imputing missing free lunch information based on the mode of each school acknowledges the contextual relevance of free lunch status within educational settings and reflects the collective socioeconomic background of students within each school. This method also ensures consistency and comparability across classrooms within the same school, enhancing the reliability and validity of the imputed data. Additionally, by leveraging the mode of each school, which represents the most prevalent free lunch status observed within the entire school population, this approach captures the overarching socioeconomic profile of the school community.
...
In the context of heterogeneity analysis within Project STAR, the data effectively ensures a balanced gender distribution across various class types, encompassing small, regular, and regular + aide classes. However, it is imperative to recognize disparities in the representation of racial and ethnic groups. To have more understanding of diversity within each school based on urbanicity, let's look at the following figure. We have school urbanicity as our x-axis, and the number of students participating in the STAR program across various schools as the y-axis. Note that the majority of students are situated in rural schools (20.2% in Inner-city, 23.2% in Suburban, 47.4% in Rural, and 9.2% in Urban). The bar plot is further segmented based on students' race, using colors corresponding to the legend (1 = White, 2 = Black, 3 = Other). The analysis reveals that White students predominate in Suburban (61%), Rural (93%), and Urban (85%) schools, while Inner-city schools are predominantly attended by Black students (96%). Due to the limited representation of other races, their participation is challenging to depict accurately; however, students identifying as Asian, Hispanic, Native American, and Other are primarily distributed among Suburban, Rural, and Urban schools. Remarkably, only three Asians enrolled in inner-city schools.We also observed that the underrepresentation of minorities is accompanied with some missing values in their math scores--an essential variable in this project. Specifically, among the 22 Asians participating in the STAR project, only 19 observations have recorded math scores, while out of nine Native Americans, only four have recorded math scores.
...
In the ANOVA table provided, there are several factors being evaluated: "Class Type," "School ID," "Free lunch," and "Race Proportion." Each factor is assessed for its contribution to explaining the variability in the dependent variable (Math score). The p-values associated with each factor indicate whether they are statistically significant predictors of the outcome variable. In this table, "Class Type," and "School ID" have p-values less than 0.05, indicating they are statistically significant predictors. On the other hand, "Race Proportion" and "Free lunch" has a p-value greater than 0.05, suggesting that given "Class Type," and "School ID" already in the model, adding "Race Proportion" and "Free lunch" did not significantly explain more variation in the model.
Since dropping non-significant predictors from the model can lead to a simpler model without sacrificing predictive accuracy, while also help in avoid overfitting, we will consider dropping "Race Proportion" and "Free lunch."

To understand why the addition of race proportion and free lunch as predictors was deemed insignificant, we can reference the scatterplot discussed in section 6.2.2. From our observations, it became evident that school ID potentially encapsulates underlying distinctions in academic performance across schools. This implies that schools exhibiting consistently higher or lower average math scores may reflect various school-level factors impacting student achievement, including teaching quality, curriculum effectiveness, socioeconomic status, and potentially race demographics. Given that this information is already encapsulated within the school ID predictor, the addition of race proportion and free lunch variables might be redundant, hence their insignificance in improving the model's predictive power.

Thus, we will proceed with the following model:
...
plot(n_model, which = 1, col = "royalblue", lty = 1, cex = 0.7, pch = 16)
```
...
Defining $\alpha = 0.05$, running this test returns a p-value of 0.087. As this p-value exceeds the threshold, we fail to reject the null hypothesis, suggesting homoscedasticity in our data and hence, satisfying the homogeneity assumption.
...
par(mfrow=c(1,2))
plot(n_model, which=2, col = "royalblue", lty = 1, cex = 1, col.axis = "darkgray",     
     col.lab = "darkgray")
hist(n_model$residuals, 
     breaks = 30,               
     col = "royalblue",           
     border = "white",         
...
The decision to replace school ID with school urbanicity in sensitivity analysis is justified by observed heterogeneity in math scores across different urbanicity categories, as discussed in the Caveats section. This suggests that urbanicity may significantly related to math scores, reflecting underlying differences in academic performance associated with different school environments. By incorporating school urbanicity as a predictor variable, we aim to assess its impact on model outcomes and evaluate its potential to provide a more accurate representation of the relationship between school characteristics and math scores. This approach enhances the robustness of our findings and ensures that conclusions are not overly influenced by specific school identifications. 
...
```{r}
average_scores_inner <- inner %>%
  group_by(g1classtype) %>%
  summarize(average_math_score = mean(g1tmathss, na.rm = TRUE))

average_scores_suburb<- suburb %>%
  group_by(g1classtype) %>%
  summarize(average_math_score = mean(g1tmathss, na.rm = TRUE))

average_scores_rural <- rural %>%
  group_by(g1classtype) %>%
  summarize(average_math_score = mean(g1tmathss, na.rm = TRUE))

average_scores_urban <- urban %>%
  group_by(g1classtype) %>%
  summarize(average_math_score = mean(g1tmathss, na.rm = TRUE))

combined_table <- bind_rows(
  data.frame(Dataset = "Inner", average_scores_inner),
  data.frame(Dataset = "Suburb", average_scores_suburb),
  data.frame(Dataset = "Rural", average_scores_rural),
  data.frame(Dataset = "Urban", average_scores_urban)
) %>%
  pivot_wider(names_from = Dataset, values_from = average_math_score)
...
We can observe from the plot that the comparison between the average score of regular and regular + aide classes still crosses the zero line, and it appears that there is still a significant difference between small-regular and small-regular + aide. 
...
Our study thoroughly examined the relationship between class type and math scores, focusing on scaled math score, class type, and school ID as primary variables. Initial variable exploration revealed missing data, issues with heterogeneity, and noncompliance, which were addressed through data imputation, additional variable inclusion, and variable substitution. Descriptive analysis confirmed minimal alterations post-imputation, ensuring data integrity. Utilizing a multiple linear regression model, we aggregated imputed data from student to teacher level to facilitate per-class observations. We used math score as the response variable and class type, school ID, race, and free lunch as independent variables. Model testing identified insignificance in additional predictors, leading to their exclusion. Diagnostic tests indicated no significant errors. Sensitivity testing involved complete case analysis, substituting school urbanicity for school ID, and replacing class type with actual size. Consistently, our analyses revealed differences in math scores across class types, with small classes consistently exhibiting the highest mean scores. Tukey HSD analysis further confirmed disparities between class averages, with small classes attaining the highest math scores.

An explanation to the result could be that smaller class sizes offer numerous advantages in education. They significantly reduce teacher workload, allowing for more focused attention on each student's needs, leading to improved instructional quality. Additionally, smaller classes create a less chaotic and more comfortable learning environment, fostering increased student engagement and performance. Strong teacher-student relationships thrive in smaller classes, contributing to a culture of high achievement. Importantly, these settings provide more one-on-one time, preventing struggling learners from falling behind academically.

Replacing free-lunch with the mode of free lunch status per school instead of per class may oversimplify the analysis and obscure individual-level socioeconomic disparities within each class. This approach may not accurately capture the socioeconomic diversity present within each class and could bias the results. Additionally, aggregating free lunch status at the school level may introduce ecological fallacy and lead to misleading interpretations. Therefore, while this approach may offer simplicity, it may not accurately capture the complexities of socioeconomic diversity within each class.

While the decision to prioritize interpretability of the untransformed model over minor deviations from normality may seem reasonable, it carries potential risks that could undermine the validity of the analysis. Firstly, relying solely on the Central Limit Theorem (CLT) to support the assumption of normality in residuals may overlook situations where the underlying data distribution deviates significantly from normality, especially with smaller sample sizes. In such cases, the CLT may not fully mitigate the impact of non-normality on the accuracy of inferential procedures. Additionally, while linear regression estimates may remain unbiased and consistent in the presence of minor deviations from normality, extreme departures from normality could lead to biased parameter estimates and inaccurate confidence intervals. Furthermore, the Shapiro-Wilk test, despite its sensitivity to sample size, serves as a valuable diagnostic tool for identifying potential violations of normality assumptions, which could warrant further investigation or model adjustments. Therefore, while prioritizing interpretability is important, it is essential to balance this with the need for rigorous adherence to statistical assumptions and robustness of inferential procedures, especially in contexts where the consequences of model misspecification could be significant.

### 12. Conclusion

Our study has provided valuable insights into the relationship between class type and math scores, revealing consistent differences across different class sizes. The findings suggest that smaller class sizes, characterized by more individualized attention and a conducive learning environment, tend to yield higher math scores compared to larger classes. These results underscore the potential benefits of smaller class sizes in enhancing academic achievement and fostering a supportive learning environment. Additionally, our discussion highlighted the importance of careful consideration when handling variables such as free lunch status and normality assumptions in statistical analyses. By acknowledging the complexities and potential pitfalls in data analysis, we can ensure that our findings are robust and reliable, contributing to a more comprehensive understanding of the factors influencing educational outcomes.
